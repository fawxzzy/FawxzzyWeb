...
- Email identifiers use Supabase password authentication directly. Username identifiers use the
  ...
  Its database RPC returns a named one-row relation rather than a scalar response, keeping the
  PostgREST representation contract explicit and preventing a valid lookup from being rejected
  during HTTP content negotiation. It resolves one exact indexed case-insensitive username,
