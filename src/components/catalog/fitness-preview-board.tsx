        ...
        {slot.imageSrc ? (
          ...
          <div className="relative z-10 flex h-full flex-col items-center justify-center gap-3 px-3 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-[0.7rem] leading-snug text-subtle">
                Drop in a screenshot at the path below.
              </p>
            </div>
          </div>
        )}
...
export function FitnessPreviewBoard({
  variant = "page",
}: FitnessPreviewBoardProps) {
  const isInline = variant === "inline";
  const visibleLayers = isInline
    ? fitnessPreviewLayers.slice(0, 2)
    : fitnessPreviewLayers;
  const stats = getFitnessPreviewStats(visibleLayers);

  return (
    <section
      aria-labelledby="fitness-preview-board-title"
      className={isInline ? "space-y-4" : "space-y-6"}
    >
      <div className="rounded-[1.75rem] border border-stroke/25 bg-panel-strong/55 p-4 shadow-[0_18px_54px_rgba(0,0,0,0.24)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0 space-y-2">
            ...
            <h2
              className="text-2xl font-semibold leading-none tracking-[-0.045em] text-text md:text-4xl"
              id="fitness-preview-board-title"
            >
              Layered screen slots
            </h2>
            <p className="max-w-3xl text-sm leading-6 text-muted">
              A single grid page for base screens, sub screens, and repeating UI
              variation layers. Every frame links back to the screen reference
              and exposes the replacement asset path.
            </p>
          </div>
        ...
        >
          <div className="flex flex-col gap-1 px-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h3
                className="text-xl font-semibold tracking-[-0.035em] text-text"
                id={`${layer.id}-title`}
              >
                {layer.title}
              </h3>
