...
function makeVariationLayer(layerNumber: number, iterationNumber: number): FitnessPreviewLayer {
  ...
  return {
    ...
    description: "Same screen map as Layer 1, reserved for the next UI pass. Replace each frame image without changing the screen reference.",
    iterationLabel,
    slots: screenTemplates.map((screen) => buildSlot(screen, layerNumber, `Layer ${layerNumber}`, iterationLabel, iterationSlug, null)),
  };
}
